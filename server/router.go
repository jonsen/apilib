...
func (r *router) Connect(pattern string, h ...Handler) Route {
	return r.addRoute("CONNECT", pattern, h)
}

func (r *router) Trace(pattern string, h ...Handler) Route {
	return r.addRoute("TRACE", pattern, h)
}
...
func (r *router) Handle(res http.ResponseWriter, req *http.Request, context martini.Context) {
	bestMatch := NoMatch
	...
	context.MapTo(c, (*martini.Context)(nil))
	c.run()
}
...
func (r *route) Handle(c martini.Context, res http.ResponseWriter) {
	context := &routeContext{c, 0, r.handlers}
	c.MapTo(context, (*martini.Context)(nil))
	c.MapTo(r, (*Route)(nil))
	context.run()
}
